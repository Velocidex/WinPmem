...
__int64 WinPmem::copy_memory(unsigned __int64 start, unsigned __int64 end) {
  ...
  while(start < end) 
  {
    ...
    // Moves the file pointer of the specified file offset.
...
__int64 WinPmem::set_write_enabled(void) {
  ...
  if(!DeviceIoControl(fd_, PMEM_WRITE_ENABLE, 
					  &mode, 4, // in
					  NULL, 0, // out
                      &size, NULL)) {
...
void WinPmem::print_memory_info() {
  ...
  if(!DeviceIoControl(fd_, PMEM_INFO_IOCTRL, 
					  NULL, 0, // in
...
__int64 WinPmem::set_acquisition_mode(unsigned __int32 mode) {
  ...
  if(!DeviceIoControl(fd_, PMEM_CTRL_IOCTRL, 
						&mode, 4, // in
						NULL, 0,  // out
                      &size, NULL)) {
...
__int64 WinPmem::write_coredump() {
  ...
  print_memory_info(); // xxx: this will double call GET_INFO IOCTL code! It is sufficient to call it one time.
